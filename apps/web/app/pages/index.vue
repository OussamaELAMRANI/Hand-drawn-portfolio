...
// ── booking calendar (current month, weekends unavailable) ──
const today = new Date()
const year = today.getFullYear()
const month = today.getMonth()
const monthLabel = today.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
const dowLabels = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']
const slotLabels = ['09:30', '11:00', '13:00', '14:30', '16:00']
...
const selectedSummary = computed(() => {
  ...
  return `You picked ${wk}, ${mo} ${selectedDay.value} · ${slotLabels[selectedSlot.value]}`
})
</script>

<template>
  <div
    class="min-h-screen bg-paper font-sans text-ink antialiased [background-image:radial-gradient(#dcdcd4_1.1px,transparent_1.1px)] [background-size:23px_23px] dark:bg-night dark:text-chalk dark:[background-image:radial-gradient(#2c2f37_1.1px,transparent_1.1px)]"
  >
    <!-- ============ NAV ============ -->
    <UiNavbar :links="navLinks" :cta="{ label: 'Hire Me', href: '#book' }" />
    ...
    <section id="book" class="mx-auto max-w-[1120px] px-6 pb-10 pt-[60px]">
      <Typography variant="h2" class="!text-[52px]">Book my time</Typography>
      <p
        class="mb-[30px] mt-2 -rotate-[0.5deg] font-hand text-[19px] text-ink-400 dark:text-chalk-500"
      >
        — recruiters &amp; clients: grab a 30-min intro. Pick a day, pick a slot, done.
      </p>
      ...
      >
        ...
        <div>
          <div class="mb-4 flex items-center justify-between">
            ...
            <div class="font-mono text-xs text-ink-300 dark:text-chalk-500">GMT+1 · 30 min</div>
          </div>
          <div
        ...
        <div class="relative">
          <div
            class="pointer-events-none absolute -left-[92px] top-6 hidden h-[60px] w-20 md:block"
          >
